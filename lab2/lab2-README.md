...
> Add new cells **above or below** this one before proceeding.
   ...
   If this statement fails or the mortgage email isn't delivered after several minutes, you can retry by stopping this statement and running the following. Again, make sure to replace `<<YOUR_EMAIL_ADDRESS_HERE>>` with your actual email address:
...
4. In a new cell, check the output of `mortgage_decisions`

   ```sql
   SELECT * FROM mortgage_decisions;
   ```

5. Checkout John's application
   ```sql
