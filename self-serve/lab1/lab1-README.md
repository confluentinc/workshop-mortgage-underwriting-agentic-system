...
We will now enrich mortgage applications with credit score data. This will create a new data product called `enriched_mortgage_applications`, which joins the `mortgage_applications` topic with the `PROD.SAMPLE.APPLICANT_CREDIT_SCORE` topic.
...
2. Navigate to the Java code directory:
   ```bash
   cd self-serve/terraform/code/FlinkTableAPI/
   ```
