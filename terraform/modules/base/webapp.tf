resource "null_resource" "webapp_container_windows" {
  ...
  triggers = {
    dockerfile_hash  = filemd5(join("/", [path.root, "..", "..", "webapp", "Dockerfile"]))
...
$envFile = Join-Path "${path.root}" ".webapp.env"
@'
...
& $runtime build -t mortgage-webapp:local "${path.root}/../../webapp"
...
EOT
  ...
  provisioner "local-exec" {
    when        = destroy
    interpreter = ["PowerShell", "-Command"]
    command     = "$runtime = if (Get-Command docker -ErrorAction SilentlyContinue) { \"docker\" } elseif (Get-Command podman -ErrorAction SilentlyContinue) { \"podman\" } else { \"\" }; if ($runtime) { & $runtime rm -f mortgage-webapp 2>$null }; $envFile = Join-Path \"${path.root}\" \".webapp.env\"; if (Test-Path $envFile) { Remove-Item -Force $envFile }"
  }
}
...
EOT
  ...
  provisioner "local-exec" {
    when        = destroy
    interpreter = ["bash", "-c"]
    command     = "if command -v docker >/dev/null 2>&1; then runtime=docker; elif command -v podman >/dev/null 2>&1; then runtime=podman; else runtime=\"\"; fi; if [ -n \"$runtime\" ]; then $runtime rm -f mortgage-webapp >/dev/null 2>&1 || true; fi; env_file=\"${path.root}/.webapp.env\"; [ -f \"$env_file\" ] && rm -f \"$env_file\""
  }
}
