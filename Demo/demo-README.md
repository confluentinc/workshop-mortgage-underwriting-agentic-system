...
- Checkout the complete mortgage underwriting result (decision + letter)
   ```sql
   SELECT * FROM mortgage_final_decisions
   WHERE borrower_name = 'Omar Soli'
   ```

> **Note:** The `mortgage_final_decisions` table now contains both the underwriting decision (including interest rate and explanation) AND the formal letter, all generated by a single AI agent workflow.
